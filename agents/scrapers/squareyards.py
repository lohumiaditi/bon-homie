...
def build_search_url(prefs: dict, page: int = 1) -> str:
    ...
    return (
        f"{BASE_URL}/pune/{area_slug}-property-for-rent"
        f"?minBudget={budget_min}&maxBudget={budget_max}&page={page}"
    )
        ...
        async with self as scraper:
            for page_num in range(1, max_pages + 1):
                ...
                if page_num < max_pages:
                    await scraper.random_delay(3.0, 5.5)
...
async def _test():
    ...
    listings = await s.scrape(prefs, max_pages=1)
