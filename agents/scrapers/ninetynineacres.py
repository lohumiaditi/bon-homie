"""
99Acres Scraper (sync)
----------------------
Scrapes rental listings from 99acres.com for Pune.
Uses Playwright sync API — no asyncio, works on all platforms.
...
def parse_listing_card(card_soup) -> dict:
    ...
    try:
        # Title
        title_el = (card_soup.select_one("a.srpTitle") or
        ...
        # URL + listing ID
        link = (card_soup.select_one("a[href*='/property/']") or
        ...
        # Price
        price_el = (card_soup.select_one("[class*='price']") or
        ...
        # Location
        loc_el = (card_soup.select_one("[class*='locality']") or
        ...
        pass  # partial data is fine — image filter handles quality
    return listing
        ...
        with self as scraper:
            for page_num in range(1, max_pages + 1):
                ...
                try:
                    ...
                    scraper.random_delay(0.8, 1.5)

                    html = page.content()
                    soup = BeautifulSoup(html, "html.parser")

                    # 99Acres uses several class patterns across redesigns
                    cards = (soup.select("div[class*='srpCard']") or
                             soup.select("div[class*='propertyCard']") or
                             soup.select("article[class*='listing']") or
                             soup.select("div[data-label='SRPLISTING']") or
                             soup.select("div[class*='body_noSERP']"))

                    if not cards:
                        print(f"  [99acres] No listing cards found on page {page_num}.")
                        print(f"  [99acres] Page title: {page.title()}")
                        break
                    ...
                    print(f"  [99acres] Error page {page_num}: {e}")
                finally:
                    ctx.close()

                if page_num < max_pages:
                    scraper.random_delay(3.0, 6.0)
...
if __name__ == "__main__":
    ...
    if listings:
        ...
        print("\nNo listings found. The site may have changed its HTML structure.")
        print("This is normal — we'll update the CSS selectors when we see the page.")
