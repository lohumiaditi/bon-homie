...
def build_search_url(prefs: dict, page: int = 1) -> str:
    ...
    url = (
        ...
        f"&City=Pune&Locality={area_slug}"
...
def parse_listing_card(card_soup) -> dict:
    ...
    try:
        title_el = card_soup.select_one(
            "h2, h3, [class*='mb-srp__card--title'], [class*='title']"
        )
        if title_el:
    ...
    def scrape(self, prefs: dict, max_pages: int = 3) -> list[dict]:
        ...
        for page_num in range(1, max_pages + 1):
            ...
            try:
                ...
                if not html:
                    print(f"  [magicbricks] Empty response on page {page_num}, stopping.")
                    break
                ...
                cards = (
                    ...
                    soup.select("div[class*='mb-srp__card']") or
                    soup.select("div[class*='PropertyCard']") or
                    soup.select("div[class*='listing']")
                )

                if not cards:
                    print(f"  [magicbricks] No cards on page {page_num}. HTML length: {len(html)}")
                    break
                ...
                print(f"  [magicbricks] Error on page {page_num}: {e}")
